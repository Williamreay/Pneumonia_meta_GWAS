...
## Exonic Cauchy aggregation of rare variants at gene level
...
## Retain only exonic variants

Exonic_rare_variants <- All_rare_variants_final %>% filter(Annotation == 'exonic')

## Merge rare variants with p values

Merged_exonic_rare_variants <- merge(Exonic_rare_variants, Rare_var_p_val, by = "SNP" )
...
Merged_exonic_rare_var_gene_anno <- merge(Merged_exonic_rare_variants, Gene_anno, by ="Gene")

genes <- sort(unique(Merged_exonic_rare_var_gene_anno$Gene))
gene_ps <- list()

for (i in genes) { gene_ps[[i]] <- c(Merged_exonic_rare_var_gene_anno[Merged_exonic_rare_var_gene_anno$Gene==i,][12])}
...
write.table(af_acat, file="Output/Exonic_var_ACAT.txt",
            row.names = F, quote = F, sep = "\t")

## Write Merged output

write.table(Merged_exonic_rare_var_gene_anno, file="Output/Exonic_rare_variant_annotation.txt",

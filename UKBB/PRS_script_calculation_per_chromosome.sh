...
## PRSice2 - PRS for pneumonia in the UKBB
...
do
  ...
  --snp rsID \
  --base /home/control/data/users/william/23andMe_pneumonia/IVW_23andMe_FinnGen_FINAL/PES_PRS/PRS/PRS_input_exMHC_pneumonia.tab.txt \
  --type bgen \
  ...
  --no-regress \
  --all-score \
  --bar-levels 0.00000005,0.00001,0.005,0.05,0.1,0.5,1 \
  --allow-inter \
  --score sum \
  --beta \
  --binary-target T \
  ...
  --thread 11 \
  --out PRS_pneumonia_self_reported_or_ICD10_chr${chr} ;
done
